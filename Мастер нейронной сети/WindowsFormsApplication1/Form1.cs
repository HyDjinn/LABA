...
namespace WindowsFormsApplication1
{
    public partial class Form1 : Form
    {
        ...
        private void button4_Click(object sender, EventArgs e)
        {
            ...
            int k1 = koff(x1), k2=koff(x2), s=koff(x1+x2);
            int k;
            if (k1 > k2) k = k1; else k = k2;
            if (s > k) k = s; 
            Net[0].Neuron[0].x = x1/k;
            Net[0].Neuron[1].x = x2/k;

            Result();
            textBox6.Text = Convert.ToString((Net[layersNumber - 1].Neuron[0].x)*k);
        }
